...
app.use(cors({
  origin: ['http://localhost','http://localhost:5174', 'http://localhost:3000','http://localhost:5184', 'http://127.0.0.1:3000', 'http://192.168.252.230:5173', "http://localhost:5001:5173", process.env.ORIGIN], 
  methods: ['GET', 'POST'],
  credentials: true
}));
...
app.post('/api/run-script', (req, res) => {
 ...
 const psScriptPath = path.join(__dirname, `generate-docker-compose-${question}-${framework}.ps1`);
...
// Powershell command to execute script with argument
  ...
  var insertcategory="insert into user_log (userid,activity_code)values(?,?)"
  con.query(insertcategory,[userId , 2],(error,result)=>{
      if(error){
          console.log(error)
          // res.send({"status":"error"})
...
app.post('/api/cleanup-docker', async (req, res) => {
  ...
  try {
    ...
    var insertcategory1="insert into user_log (userid,activity_code)values(?,?)"
