...
IMAGE_NAME="sankarkg/a1l1q2-vue:latest"
...
services:
  code-server:
    container_name: "code-server-a1l1q2-vue-${USER_ID}"
    image: "$IMAGE_NAME"
    ports:
      - "${DOCKER_PORT}:8080"
      - "${OUTPUT_PORT}:5173"
...
COMPOSE_FILE_NAME="docker-compose-a1l1q2-vue-${USER_ID}.yml"
...
docker compose -f "$COMPOSE_FILE_NAME" -p a1l1q2-vue-${USER_ID} up -d
